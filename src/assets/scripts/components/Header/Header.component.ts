...
class Header extends Component {
  ...
  boot(instance: ConnectorItem) {
    setTimeout(() => {
      ...
      this.offsetTop = instance.element.offsetTop + instance.element.parentElement.offsetTop
      this.height = instance.element.offsetHeight

      if (instance.type === 'sticky') {
        if (window.scrollY >= this.offsetTop && !instance.element.classList.contains('is-sticky')) {
