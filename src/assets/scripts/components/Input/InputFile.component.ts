...
class InputFile extends Component {
  ...
  boot(instance: ConnectorItem): void {
    const input = instance.element.querySelector(
      "input[type='file']"
    ) as HTMLInputElement;
  ...
  render(instance: InputFileInstance) {
    ...
    file.innerHTML = `
            ${
              instance.label
                ? `<span class="file__label">${instance.label}</span>`
                : ""
            }
  ...
  handleEvent(instance: InputFileInstance) {
    ...
    instance.element
      ...
      .addEventListener("change", (e) => this.onChange(e, instance));
    window.addEventListener(
      "keydown",
      (e) => instance.isFocused && this.onKeyDown(e, instance)
    );
  }
  ...
  onChange(e: Event, instance: InputFileInstance) {
    const width =
      instance.faker.querySelector(".file__uploaded").getBoundingClientRect()
        .width - 20;
    ...
    if (
      instance.faker.querySelectorAll(".file__uploaded--tag").length <
