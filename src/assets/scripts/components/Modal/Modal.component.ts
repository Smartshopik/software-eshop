...
class Modal extends Component {
  ...
  defaults(options: ModalOptions = {}): void {
    ...
    this.wrapper = document.querySelector(
      options.wrapper ? options.wrapper : "#modal"
    );
  ...
  booted(): void {
    ModalGallery.register(
      Connector.get("modal", { type: "gallery" }),
  ...
  close(instance: ConnectorItem) {
    ...
    Connector.get("modal", { type: "visible", id: instance.id }).forEach(
      (conn: ConnectorItem) => {
  ...
  open(instance: ConnectorItem) {
    ...
    Connector.get("modal", { type: "visible", id: instance.id }).forEach(
      (conn: ConnectorItem) => {
        ...
        conn.element.style.zIndex = (
          parseInt(getComputedStyle(this.wrapper).zIndex) + 1
        ).toString();
      }
    );
