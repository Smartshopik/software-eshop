...
class Ajax {
  ...
  loading(instance: ConnectorItem) {
    ...
    Connector.get(Template.id, {
      id: instance.id,
    }).forEach((conn: ConnectorItem) => conn.element.classList.toggle('is-loading'))
  ...
  async process(instance: ConnectorItem, data: any) {
    if (this.options.resources[instance.event.params[0]]) {
      (this.options.resources[instance.event.params[0]] as any).call(instance, data, {
  ...
  eventFunction(e: Event, instance: ConnectorItem) {
    if (instance.element.nodeName !== 'INPUT' && (instance.element as HTMLInputElement).type !== 'checkbox') {
      e.preventDefault()
    }
    ...
    try {
      Object.entries(JSON.parse(instance.data.data ? instance.data.data : '{}')).forEach(([key, value]) => {
